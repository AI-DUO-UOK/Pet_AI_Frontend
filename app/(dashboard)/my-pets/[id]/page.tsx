    ...
    <div className="space-y-6">
      ...
      <div className="overflow-hidden bg-white border shadow-sm dark:bg-slate-900 rounded-2xl border-slate-200 dark:border-slate-800">
        <div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-3">
          <div className="flex flex-col items-center md:col-span-1">
            ...
            <div className="mt-3 inline-flex items-center gap-1.5 px-4 py-1.5 bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 rounded-full text-sm font-medium shadow-sm">
      ...
      <div className="p-6 bg-white border shadow-sm dark:bg-slate-900 rounded-2xl border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Scheduled visits for {petDetails.name}</p>
          </div>
        </div>
        ...
        {upcomingAppointments.length > 0 ? (
            ...
            {upcomingAppointments.map((appointment, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative flex items-start gap-4 p-4 rounded-xl border transition-all hover:shadow-md ${
                    isFirst
                      ? 'border-primary-200 dark:border-primary-800 bg-gradient-to-r from-primary-50/80 to-white dark:from-primary-900/10 dark:to-slate-900'
                      : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900'
                  }`}
                >
                  {isFirst && (
                    <div className="absolute top-0 right-4 px-2.5 py-0.5 bg-primary-600 text-white text-[10px] font-bold uppercase tracking-wider rounded-bl-lg rounded-br-lg">
                      Next
                    </div>
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-1.5 mt-1">
                      <span className="inline-flex items-center gap-1 text-sm font-medium text-slate-700 dark:text-slate-200">
                        ...
                        {appointment.clinic || appointment.clinic_name || appointment.clinic_id || 'Clinic'}
                      </span>
                    ...
                    <div className="flex items-center gap-1.5 mt-2 text-sm text-slate-500 dark:text-slate-400">
                      ...
                      <span>{formatDateShort(appointment.appointment_date || appointment.date)} at {appointment.appointment_time || appointment.time || ''}</span>
                    </div>
                  </div>
                </motion.div>
              );
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">No upcoming appointments</p>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Schedule a vet visit to see it here</p>
          </div>
        )}
      </div>
      ...
      <div className="p-6 bg-white border shadow-sm dark:bg-slate-900 rounded-2xl border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Channelling History</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">{pastAppointments.length} past visit{pastAppointments.length !== 1 ? 's' : ''} recorded</p>
          </div>
        </div>
        ...
        {pastAppointments.length > 0 ? (
          <div className="space-y-3">
            {pastAppointments.map((appointment, idx) => {
              ...
              const statusColor = (appointment.status || '').toLowerCase() === 'completed' ? 'emerald' :
                                 (appointment.status || '').toLowerCase() === 'cancelled' ? 'red' : 'amber';
              return (
                <motion.div
                  key={appointment.id}
                  initial={{ opacity: 0, y: 10 }}
                  ...
                  className={`relative overflow-hidden rounded-xl border transition-all hover:shadow-md ${
                    statusColor === 'emerald'
                      ? 'border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50/50 to-white dark:from-emerald-900/5 dark:to-slate-900'
                      : statusColor === 'red'
                      ? 'border-red-200 dark:border-red-800 bg-gradient-to-r from-red-50/50 to-white dark:from-red-900/5 dark:to-slate-900'
                      : 'border-amber-200 dark:border-amber-800 bg-gradient-to-r from-amber-50/50 to-white dark:from-amber-900/5 dark:to-slate-900'
                  }`}
                >
                  {/* Left accent bar */}
                  ...
                  <div className="flex items-start justify-between gap-4 p-4 pl-5">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`p-1.5 rounded-lg ${
                          statusColor === 'emerald' ? 'bg-emerald-100 dark:bg-emerald-900/30' :
                          statusColor === 'red' ? 'bg-red-100 dark:bg-red-900/30' : 'bg-amber-100 dark:bg-amber-900/30'
                        }`}>
                          <Calendar className={`w-4 h-4 ${
                            statusColor === 'emerald' ? 'text-emerald-600 dark:text-emerald-400' :
                            statusColor === 'red' ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'
                          }`} />
                        </div>
                        <h3 className="font-semibold text-slate-900 dark:text-white truncate">{appointment.type}</h3>
                        <span className={`px-2 py-0.5 text-[11px] font-semibold rounded-full ${statusMeta.className}`}>
                          {statusMeta.label}
                        </span>
                      </div>
                      ...
                      <p className="flex items-center gap-1.5 mt-1.5 text-sm text-slate-500 dark:text-slate-400">
                        <Calendar className="w-3.5 h-3.5" />
                        <span>{formatDateShort(appointment.appointment_date || appointment.date)} at {appointment.appointment_time || appointment.time || ''}</span>
                      </p>

                      {appointment.notes && (
                        <div className="relative mt-3 p-3 rounded-lg bg-white/70 dark:bg-slate-800/70 border border-slate-200/70 dark:border-slate-700/70">
                          <div className="absolute -top-2 left-3 w-3 h-3 rotate-45 bg-white/70 dark:bg-slate-800/70 border-l border-t border-slate-200/70 dark:border-slate-700/70" />
                      ...
                      {appointment.reviewed && appointment.review && (
                        <div className="relative mt-3 p-3 rounded-lg bg-amber-50/80 dark:bg-amber-900/20 border border-amber-200/70 dark:border-amber-800/70">
                          <div className="absolute -top-2 left-3 w-3 h-3 rotate-45 bg-amber-50/80 dark:bg-amber-900/20 border-l border-t border-amber-200/70 dark:border-amber-800/70" />
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Your Review</span>
                          </div>
                          <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                            <span className="text-xs text-slate-500 dark:text-slate-400">Treatment:</span> {appointment.review?.treatment}
                          </p>
                    ...
                    {canReviewAppointment(appointment) && (
                      <button
                        ...
                        className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white transition-all rounded-lg bg-primary-600 hover:bg-primary-700 hover:shadow-md hover:shadow-primary-600/20 whitespace-nowrap"
                      >
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">No channelling history</p>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Past appointments will appear here</p>
          </div>
        )}
      </div>
                  ...
                  <div className="p-4 space-y-4 border rounded-lg bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="col-span-2">
                        ...
                        <SearchableSelect
                          options={editForm.type.toLowerCase() === 'cat' ? CAT_BREEDS : DOG_BREEDS}
                          value={editForm.breed}
                          onChange={(val) => setEditForm((prev: any) => ({ ...prev, breed: val }))}
                        />
                      </div>
